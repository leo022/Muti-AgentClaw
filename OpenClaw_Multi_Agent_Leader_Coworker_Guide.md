...
> keep one agent responsible for strategy and final judgment, while specialist agents execute narrow, high-signal work.
...
### 4.5 Make workers narrow and explicit
...
5. Route external channels only where the business case exists
6. Validate with CLI output before exposing the system
7. Publish only masked configuration and sanitized outputs
